	...
	class cart
	{
		...
		public function add_to_cart($quantity,$id)
		{
			...
			$productName = $result["productName"];
				...
				{
					$msg = "<span>Product Already Added</span>";
 					return $msg;
				}else
				{
		...
		public function del_product_cart($cartid)
		{
			...
			{
				$msg = "<span class='error'>Product Quantity Deleted Not Successfully</span>";
				return $msg;
			}
			
		}public function del_order_cart($id)
		{
			$cid = mysqli_real_escape_string($this->db->link,$id);
			$query = "DELETE  FROM tbl_order WHERE id = '$id'";
			$result = $this->db->delete($query);
			if ($result) {
				header('Location: orderdetails.php.php');
		...
		public function shifted($id, $time, $price)
		{
			...
			$query = "UPDATE tbl_order SET 
					...
					{
						$msg = "<span class='error'>Updated Order Not Successfully</span>";
						return $msg;
					}
		}
		...
		{
			$sId = session_id();
			$query = "DELETE FROM tbl_compare WHERE customer_id = '$sId'";
			$result = $this->db->delete($query);
			return $result;
		}
	}
